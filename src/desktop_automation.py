...
class AutomotivApp:
    ...
    def _fill_password_field(self) -> None:
        """Localiza o campo de login por imagem, navega até senha e preenche."""
        ...
        if not clicked:
            raise RuntimeError(
                "Não foi possível localizar o campo de login por imagem. "
                "Verifique a imagem configurada em images.input_nome_login."
            )

        time.sleep(0.5)

        # Na tela atual, após clicar no campo de usuário, TAB leva para senha.
    ...
    def click_login(self) -> None:
        """Clica sempre no botão LOGIN por imagem."""
    ...
    def _send_menu_sequence(self, labels: list[str]) -> None:
        """Abre menus por imagem quando configurado, com fallback por controle Windows."""
    ...
    def search_material(self, code_or_reference: str, inactive: bool = False) -> bool:
        """Pesquisa material e valida o resultado exportando a grid para Excel.

        O Excel é salvo em um caminho conhecido, lido pelo robô e removido ao final.
        """
        ...
        time.sleep(1)
        self._set_material_status_inactive()
        ...
        try:
            material = self.find_material_in_exported_excel(
                excel_path=str(exported_file),
                expected_code=code_or_reference,
            )
    ...
    def _set_material_status_inactive(self) -> None:
        """Seleciona o status Inativo.

        Preferencialmente por imagem. Se não houver imagem, usa navegação por teclado.
        """
        keyboard.send_keys("{TAB}")
        time.sleep(0.3)
        keyboard.send_keys("{TAB}")
    ...
    def _type_search_text(self, text: str) -> None:
        """Preenche o campo Pesquisar da tela aberta."""
    ...
    def _export_grid_to_excel(self, expected_code: str) -> Path:
        """Clica no botão Excel, preenche o Salvar como e retorna o caminho salvo."""
        ...
        self.logger.info("Arquivo exportado salvo em: %s", export_path)
        return export_path
    ...
    def _click_export_excel_button(self) -> None:
        ...
        clicked = self.image.click_bottommost(
            image_name=image_name,
            timeout=10,
            confidence=self.config.images.confidence,
        )
    ...
    def _save_exported_excel_dialog(self, expected_code: str, timeout: int = 15) -> Path:
        """Preenche a janela 'Salvar como' com caminho completo e salva o arquivo."""
        ...
        file_path = download_dir / f"automotiv_material_{safe_code}_{timestamp}.xlsx"

        self.logger.info("Salvando Excel exportado em: %s", file_path)

        # O campo "Nome" já costuma vir selecionado. Colar o caminho completo resolve pasta + nome.
    ...
    def _handle_save_confirmation_if_exists(self) -> None:
        """Clica em 'Sim' caso o Windows pergunte se deseja substituir arquivo."""

        time.sleep(0.8)

        try:
            self.logger.info("Validando se apareceu mensagem para confirmar e substituir arquivo ja existente")
            self._click_configured_image_or_fail(
                image_key="modal_duplicado",
                timeout=10,
                confidence=self._get_confidence(),
            )
            self.logger.info("Apareceu a mensagem, clicando em OK")
            self._click_configured_image_or_fail(
                image_key="button_ok_duplicad",
                timeout=10,
                confidence=self._get_confidence(),
            )

        except Exception:
            self.logger.info("Não apareceu mensagem de substituicao. Seguindo...")

    def find_material_in_exported_excel(self, excel_path: str | Path, expected_code: str) -> dict[str, Any] | None:
        """Lê o Excel exportado e procura o código exato na coluna '*Código Interno'."""
    ...
    def handle_optional_message_modal(self) -> None:
        """Se o modal de mensagem aparecer, clica no OK. Se não aparecer, segue o fluxo."""
        ...
        modal_appeared = self.image.exists(
            image_name=modal_image,
            timeout=3,
            confidence=self._get_confidence(),
        )
    ...
    def _click_configured_image_or_fail(
        self,
        image_key: str,
        timeout: int = 10,
        ...
        if not image_name:
            raise RuntimeError(
                f"Imagem '{image_key}' não configurada em config/config.yaml na seção images."
            )

        clicked = self.image.click(
            image_name=image_name,
            timeout=timeout,
            confidence=confidence or self._get_confidence(),
        )
    ...
    def _get_required_image_name(self, image_key: str) -> str:
        ...
        if not image_name:
            raise RuntimeError(
                f"Imagem obrigatória '{image_key}' não configurada em config/config.yaml."
            )
    ...
    def _load_image_config(self) -> dict[str, Any]:
        """Lê a seção images diretamente do YAML.

        Isso permite adicionar novas imagens no config/config.yaml sem alterar src/config.py.
        """
    ...
    def _sanitize_filename(value: str) -> str:
        ...
        return sanitized.strip() or "sem_codigo"

    def _delete_file_safely(self, file_path: Path) -> None:
        try:
            file_path.unlink(missing_ok=True)
            self.logger.info("Arquivo temporário removido: %s", file_path)
        except Exception as exc:
            self.logger.warning("Não consegui remover arquivo temporário %s: %s", file_path, exc)
